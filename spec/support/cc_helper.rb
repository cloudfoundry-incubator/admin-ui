...
module CCHelper
  ...
  def cc_clear_users_cache_stub(config)
    ...
    sql(config.ccdb_uri, 'DELETE FROM spaces_supporters')
    sql(config.ccdb_uri, 'DELETE FROM organizations_auditors')
    sql(config.ccdb_uri, 'DELETE FROM organizations_billing_managers')
    sql(config.ccdb_uri, 'DELETE FROM organizations_managers')
    sql(config.ccdb_uri, 'DELETE FROM organizations_users')
  ...
  def cc_space_supporter
    {
      created_at:           unique_time('cc_space_supporter_created'),
      role_guid:            'SpaceSupporter1',
      spaces_supporters_pk: unique_id('cc_space_supporter'),
      space_id:             cc_space[:id],
      updated_at:           unique_time('cc_space_supporter_updated'),
      user_id:              cc_user[:id]
    }
  end
  ...
  def ccdb_inserts(insert_second_quota_definition, event_type, use_route)
    result = [
               ...
               [:spaces_supporters,                cc_space_supporter],
  ...
  def cc_space_stubs(config)
    ...
    allow(AdminUI::Utils).to receive(:http_request).with(anything, anything, "#{config.cloud_controller_uri}/v3/roles/#{cc_space_supporter[:role_guid]}", AdminUI::Utils::HTTP_DELETE, anything, anything, anything, anything, anything) do
      if @cc_spaces_deleted
        cc_space_not_found
      else
        sql(config.ccdb_uri, "DELETE FROM spaces_supporters WHERE space_id = '#{cc_space[:id]}' AND user_id = '#{cc_user[:id]}'")
        Accepted.new({})
      end
    end
