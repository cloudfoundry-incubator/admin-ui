Sequel.migration do
  change do
    ...
    create_table(:spaces_supporters, :ignore_index_errors=>true) do
      primary_key :spaces_supporters_pk
      String :role_guid, :size=>255
      foreign_key :space_id, :spaces, :null=>false, :key=>[:id]
      foreign_key :user_id, :users, :null=>false, :key=>[:id]
      DateTime :created_at, :default=>Sequel::CURRENT_TIMESTAMP, :null=>false
      ...
      index [:role_guid]
      index [:updated_at]
      index [:space_id, :user_id], :name=>:spaces_supporters_user_space_index, :unique=>true
    end
