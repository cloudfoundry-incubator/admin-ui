...
module AdminUI
  class Stats
    ...
    def initialize(config, logger, cc, varz)
      ...
      @cc              = cc
      @varz            = varz
      @persistence = AdminUI::DatabasePersistence.create_storage(config, logger)
    ...
    def stats
      ...
      begin
        ...
        @logger.debug("AdminUI::Stats.stats: Retrieved #{items}")
        result['items'] = items
      rescue => error
        @logger.debug("AdminUI::Stats.stats: Error reading stats file: #{ error }")
      end
    ...
    def schedule_stats
      ...
      @logger.debug("AdminUI::Stats.schedule_stats: Wait #{ wait_time } seconds before fetching and saving stats data.")
      sleep(wait_time)
      generate_stats
      wait_time
    rescue => error
    ...
    def save_stats(stats)
      ...
      unless stats.nil?
        begin
          @logger.debug("AdminUI::Stats.save_stats: Writing stats: #{ stats }")
