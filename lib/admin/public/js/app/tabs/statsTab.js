...
{
    this.table = Table.createTable(this.id, this.getColumns(), this.getInitialSort(), null, this.getActions());
};
...
{
    var historyDeferred = Data.get(Constants.URL__STATS,         reload);
    var currentDeferred = Data.get(Constants.URL__CURRENT_STATS, reload);

    $.when(historyDeferred, currentDeferred).done($.proxy(function(historyResult, currentResult)
        {
            var result = historyResult;
            result.response.items.unshift(currentResult.response);
