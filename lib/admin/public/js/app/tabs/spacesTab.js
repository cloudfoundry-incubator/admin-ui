...
SpacesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[9] != null)
    {
        this.addFilterRow(table, "Default Users", Format.formatNumber(row[9]), Format.formatString(row[3]), AdminUI.showUsers);
    }
    ...
    if (row[11] != null)
    {
        this.addFilterRow(table, "Private Service Brokers", Format.formatNumber(row[11]), Format.formatString(row[3]), AdminUI.showServiceBrokers);
    }
    
    if (row[12] != null)
    {
        this.addFilterRow(table, "Security Groups", Format.formatNumber(row[12]), space.guid, AdminUI.showSecurityGroupsSpaces);
    }
    
    if (row[13] != null)
    {
        this.addFilterRow(table, "Total Routes", Format.formatNumber(row[13]), Format.formatString(row[3]), AdminUI.showRoutes);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Used Routes", Format.formatNumber, row[14]);
    this.addRowIfValue(this.addPropertyRow, table, "Unused Routes", Format.formatNumber, row[15]);

    if (row[16] != null)
    {
        this.addFilterRow(table, "Instances Used", Format.formatNumber(row[16]), Format.formatString(row[3]), AdminUI.showApplicationInstances);
    }

    if (row[17] != null)
    {
        this.addFilterRow(table, "Services Used", Format.formatNumber(row[17]), Format.formatString(row[3]), AdminUI.showServiceInstances);
    }
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Memory Reserved", Format.formatNumber, row[21]);
    this.addRowIfValue(this.addPropertyRow, table, "Disk Reserved",   Format.formatNumber, row[22]);

    if (row[23] != null)
    {
        this.addFilterRow(table, "Total Apps", Format.formatNumber(row[23]), Format.formatString(row[3]), AdminUI.showApplications);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Started Apps", Format.formatNumber, row[24]);
    this.addRowIfValue(this.addPropertyRow, table, "Stopped Apps", Format.formatNumber, row[25]);
    this.addRowIfValue(this.addPropertyRow, table, "Pending Apps", Format.formatNumber, row[26]);
    this.addRowIfValue(this.addPropertyRow, table, "Staged Apps", Format.formatNumber, row[27]);
    this.addRowIfValue(this.addPropertyRow, table, "Failed Apps", Format.formatNumber, row[28]);
};
