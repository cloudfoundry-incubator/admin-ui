...
SpacesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[9] != null)
    {
        this.addFilterRow(table, "Total Routes", Format.formatNumber(row[9]), Format.formatString(row[3]), AdminUI.showRoutes);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Used Routes", Format.formatNumber, row[10]);
    this.addRowIfValue(this.addPropertyRow, table, "Unused Routes", Format.formatNumber, row[11]);

    if (row[12] != null)
    {
        this.addFilterRow(table, "Instances Used", Format.formatNumber(row[12]), Format.formatString(row[3]), AdminUI.showApplications);
    }

    if (row[13] != null)
    {
        this.addFilterRow(table, "Services Used", Format.formatNumber(row[13]), Format.formatString(row[3]), AdminUI.showServiceInstances);
    }
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Memory Reserved", Format.formatNumber, row[17]);
    this.addRowIfValue(this.addPropertyRow, table, "Disk Reserved",   Format.formatNumber, row[18]);

    if (row[19] != null)
    {
        this.addFilterRow(table, "Total Apps", Format.formatNumber(row[19]), Format.formatString(row[3]), AdminUI.showApplications);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Started Apps", Format.formatNumber, row[20]);
    this.addRowIfValue(this.addPropertyRow, table, "Stopped Apps", Format.formatNumber, row[21]);
    this.addRowIfValue(this.addPropertyRow, table, "Pending Apps", Format.formatNumber, row[22]);
    this.addRowIfValue(this.addPropertyRow, table, "Staged Apps", Format.formatNumber, row[23]);
    this.addRowIfValue(this.addPropertyRow, table, "Failed Apps", Format.formatNumber, row[24]);
};
