...
SpacesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if ((labels != null) && (labels.length > 0))
    {
        ...
        for (var labelIndex = 0; labelIndex < labels.length; labelIndex++)
        {
            ...
            spaceLabelRow.push(label.resource_guid + "/labels/" + encodeURIComponent(label.key_prefix) + "/" + encodeURIComponent(label.key_name));
            spaceLabelRow.push(label.key_prefix);
            spaceLabelRow.push(label.key_name);
            spaceLabelRow.push(label.guid);
            spaceLabelRow.push(wrapper.created_at_rfc3339);
            spaceLabelRow.push(wrapper.updated_at_rfc3339);
            spaceLabelRow.push(label.value);

            spacesLabelsTableData.push(spaceLabelRow);
        }

        // Have to show the table prior to populating for its sizing to work correctly.
        $("#SpacesLabelsTableContainer").show();

        this.spacesLabelsTable.api().clear().rows.add(spacesLabelsTableData).draw();
    }
};
