...
SpacesTab.prototype.getActions = function()
{
    return [
               ...
               },
               {
                   ...
                   click: $.proxy(function()
                   {
                       this.deleteChecked("Are you sure you want to remove the selected spaces' isolation segments?",
                                          "Remove",
                                          "Managing Spaces",
                                          Constants.URL__SPACES,
                                          "/isolation_segment");
               ...
               },
               {
                   ...
                   click: $.proxy(function()
                   {
                       this.deleteChecked("Are you sure you want to delete the selected spaces and their contained applications, routes, route mappings, private service brokers, service instances, service bindings, service keys and route bindings?",
                                          "Delete Recursive",
...
{
    var isolation_segment = objects.isolation_segment;
    ...
    this.addFilterRowIfValue(table, "Total Apps", Format.formatNumber, row[17], Format.formatString(row[3]), AdminUI.showApplications);
    this.addFilterRowIfValue(table, "Instances Used", Format.formatNumber, row[18], Format.formatString(row[3]), AdminUI.showApplicationInstances);
    this.addFilterRowIfValue(table, "Services Used", Format.formatNumber, row[19], Format.formatString(row[3]), AdminUI.showServiceInstances);
    this.addFilterRowIfValue(table, "Tasks Used", Format.formatNumber, row[20], Format.formatString(row[3]), AdminUI.showTasks);
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Desired Started Apps", Format.formatNumber, row[26]);
    this.addRowIfValue(this.addPropertyRow, table, "Desired Stopped Apps", Format.formatNumber, row[27]);
    ...
    if (isolation_segment != null)
    {
        this.addFilterRow(table, "Isolation Segment", Format.formatStringCleansed(isolation_segment.name), isolation_segment.guid, AdminUI.showIsolationSegments);
        this.addPropertyRow(table, "Isolation Segment GUID", Format.formatString(isolation_segment.guid));
    }
};
