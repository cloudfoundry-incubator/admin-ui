...
ApplicationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[12] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[12]), application.guid, AdminUI.showEvents);
    }
    
    if (row[13] != null)
    {
        this.addFilterRow(table, "Instances", Format.formatNumber(row[13]), application.guid, AdminUI.showApplicationInstances);
    }
    
    if (row[14] != null)
    {
        this.addFilterRow(table, "Service Bindings", Format.formatNumber(row[14]), application.guid, AdminUI.showServiceBindings);
    }
