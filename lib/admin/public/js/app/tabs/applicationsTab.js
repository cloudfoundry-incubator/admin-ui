    ...
    return [
               {
                   ...
                   {
                       return "<input type='checkbox' name='" + escape(item[1]) + "' value='" + value + "' onclick='ApplicationsTab.prototype.checkboxClickHandler(event)'></input>";
                   }
               },
               {
                   "sTitle": "Name",
    ...
    return [
               {
                   ...
                   "mRender": function(name, type, item)
                   {
                       if (Format.doFormatting(type))
                       {
                           return Format.formatApplicationName(name, type) + 
...
ApplicationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[19] != null)
    {
        this.addFilterRow(table, "DEA", Format.formatStringCleansed(row[19]), row[19], AdminUI.showDEAs);
    }
...
ApplicationsTab.prototype.manageApplications = function(operation)
{
    ...
    var callback = $.proxy(function()
    {
        ...
        for (var appIndex = 0; appIndex < apps.length; appIndex++)
        {
            ...
            var url = Constants.URL__APPLICATIONS + "/" + app.guid;
            ...
            {
                this.sendAjaxRequest(app.name, "DELETE", url, null, failCallback, alwaysCallback);
            }
        }
    }, this);
