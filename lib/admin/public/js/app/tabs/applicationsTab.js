...
ApplicationsTab.prototype.manageApplications = function(operation)
{
    ...
    AdminUI.showModalDialog("Managing Applications", $("<label>Performing operation, please wait...</label>"));
    ...
    try
    {
        for (var index = 0; index < apps.length; index++)
        {
            ...
            if (operation == "start")
            {
                this.sendSyncRequest("PUT", url, '{"state":"STARTED"}', failCallback);
            }
            else if (operation == "stop")
            {
                this.sendSyncRequest("PUT", url, '{"state":"STOPPED"}', failCallback);
            }
            ...
            {
                this.sendSyncRequest("DELETE", url, null, failCallback);
            }
        }
    }
    finally
...
ApplicationsTab.prototype.sendSyncRequest = function(type, url, body, failCallback)
{
    var deferred = $.ajax({
                              type:        type,
                              async:       false,
...
ApplicationsTab.prototype.getSelectedApps = function()
{
    ...
    {
        apps.push(checkedRows[index].value);
    }
