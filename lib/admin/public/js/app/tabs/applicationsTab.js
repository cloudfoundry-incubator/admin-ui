...
ApplicationsTab.prototype.initialize = function()
{
    ...
    this.applicationsAnnotationsTable = Table.createTable("ApplicationsAnnotations", this.getApplicationsAnnotationsColumns(), [[1, "asc"]], null, this.getApplicationsAnnotationsActions(), Constants.FILENAME__APPLICATION_ANNOTATIONS, null, null);
    ...
    return [
               {
                   ...
                   render:    $.proxy(function(value, type, item)
                                      {
                                          return this.formatCheckbox("ApplicationsAnnotations", item[1], value);
                                      },
                                      this)
               },
               {
...
ApplicationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    {
        var applicationsLabelsTableData = [];
        ...
        for (var labelIndex = 0; labelIndex < labels.length; labelIndex++)
        {
            ...
            var applicationLabelRow = [];

            applicationLabelRow.push(label.resource_guid + "/labels/" + encodeURIComponent(label.key_prefix) + "/" + encodeURIComponent(label.key_name));
            applicationLabelRow.push(label.key_prefix);
            applicationLabelRow.push(label.key_name);
            applicationLabelRow.push(label.guid);
            applicationLabelRow.push(wrapper.created_at_rfc3339);
            applicationLabelRow.push(wrapper.updated_at_rfc3339);
            applicationLabelRow.push(label.value);

            applicationsLabelsTableData.push(applicationLabelRow);
        }

        // Have to show the table prior to populating for its sizing to work correctly.
        $("#ApplicationsLabelsTableContainer").show();

        this.applicationsLabelsTable.api().clear().rows.add(applicationsLabelsTableData).draw();
    }

    if ((annotations != null) && (annotations.length > 0))
    {
        var applicationsAnnotationsTableData = [];
        ...
        for (var annotationIndex = 0; annotationIndex < annotations.length; annotationIndex++)
        {
            ...
            var applicationAnnotationRow = [];

            applicationAnnotationRow.push(label.resource_guid + "/annotations/" + encodeURIComponent(annotation.key));
            applicationAnnotationRow.push(annotation.key);
            applicationAnnotationRow.push(annotation.guid);
            applicationAnnotationRow.push(wrapper.created_at_rfc3339);
            applicationAnnotationRow.push(wrapper.updated_at_rfc3339);
            applicationAnnotationRow.push(annotation.value);

            applicationsAnnotationsTableData.push(applicationAnnotationRow);
        }

        // Have to show the table prior to populating for its sizing to work correctly.
        $("#ApplicationsAnnotationsTableContainer").show();

        this.applicationsAnnotationsTable.api().clear().rows.add(applicationsAnnotationsTableData).draw();
    }
