...
ApplicationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    this.addPropertyRow(table, "SSH Enabled", Format.formatBoolean(row[10]));
    ...
    if (row[13] != null)
    {
        this.addFilterRow(table, "Buildpack GUID", Format.formatString(row[13]), row[13], AdminUI.showBuildpacks);
    }
    ...
    if (row[14] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[14]), application.guid, AdminUI.showEvents);
    }
    
    if (row[15] != null)
    {
        this.addFilterRow(table, "Instances", Format.formatNumber(row[15]), application.guid, AdminUI.showApplicationInstances);
    }
    
    if (row[16] != null)
    {
        this.addFilterRow(table, "Service Bindings", Format.formatNumber(row[16]), application.guid, AdminUI.showServiceBindings);
    }
