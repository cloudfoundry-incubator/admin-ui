...
ApplicationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[10] != null)
    {
        ...
        {
            this.addFilterRow(table, "Stack", Format.formatStringCleansed(row[10]), row[10], AdminUI.showStacks);
        }
    }
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Instance Index", Format.formatNumber, row[12]);
    ...
    if (row[13] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[13]), row[2], AdminUI.showEvents);
    }
    
    this.addRowIfValue(this.addPropertyRow, table, "Services Used", Format.formatNumber, row[14]);
    ...
    this.addPropertyRow(table, "Memory Reserved",  Format.formatNumber(row[18]));
    this.addPropertyRow(table, "Disk Reserved",    Format.formatNumber(row[19]));
    ...
    if (row[21] != null)
    {
        this.addFilterRow(table, "DEA", Format.formatStringCleansed(row[21]), row[21], AdminUI.showDEAs);
    }
