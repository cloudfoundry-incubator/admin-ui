...
ServiceBrokersTab.prototype.showDetails = function(table, serviceBroker, row)
{
    ...
    if (row[6] != null)
    {
        this.addFilterRow(table, "Services", Format.formatNumber(row[6]), serviceBroker.guid, AdminUI.showServices);
    }
    
    if (row[7] != null)
    {
        this.addFilterRow(table, "Service Plans", Format.formatNumber(row[7]), serviceBroker.guid, AdminUI.showServicePlans);
    }
    
    if (row[8] != null)
    {
        this.addFilterRow(table, "Service Plan Visibilities", Format.formatNumber(row[8]), serviceBroker.guid, AdminUI.showServicePlanVisibilities);
    }
    
    if (row[9] != null)
    {
        this.addFilterRow(table, "Service Instances", Format.formatNumber(row[9]), serviceBroker.guid, AdminUI.showServiceInstances);
    }
    
    if (row[10] != null)
    {
        this.addFilterRow(table, "Service Bindings", Format.formatNumber(row[10]), serviceBroker.guid, AdminUI.showServiceBindings);
    }
    
    if (row[11] != null)
    {
        this.addFilterRow(table, "Service Keys", Format.formatNumber(row[11]), serviceBroker.guid, AdminUI.showServiceKeys);
    }
};
