...
ServiceBrokersTab.prototype.showDetails = function(table, serviceBroker, row)
{
    ...
    if (row[5] != null)
    {
        this.addFilterRow(table, "Services", Format.formatNumber(row[5]), serviceBroker.guid, AdminUI.showServices);
    }
    
    if (row[6] != null)
    {
        this.addFilterRow(table, "Service Plans", Format.formatNumber(row[6]), serviceBroker.guid, AdminUI.showServicePlans);
    }
    
    if (row[7] != null)
    {
        this.addFilterRow(table, "Service Plan Visibilities", Format.formatNumber(row[7]), serviceBroker.guid, AdminUI.showServicePlanVisibilities);
    }
    
    if (row[8] != null)
    {
        this.addFilterRow(table, "Service Instances", Format.formatNumber(row[8]), serviceBroker.guid, AdminUI.showServiceInstances);
    }
    
    if (row[9] != null)
    {
        this.addFilterRow(table, "Service Bindings", Format.formatNumber(row[9]), serviceBroker.guid, AdminUI.showServiceBindings);
    }
};
