...
CellsTab.prototype.showDetails = function(table, cell, row)
{
    ...
    if (row[11] != null)
    {
        this.addFilterRow(table, "Used Containers", Format.formatNumber(row[11]), Format.formatString(row[0]), AdminUI.showApplicationInstances);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Memory Capacity MB", Format.formatNumber, row[12]);
    this.addRowIfValue(this.addPropertyRow, table, "Memory Remaining MB", Format.formatNumber, row[13]);
    this.addRowIfValue(this.addPropertyRow, table, "Disk Capacity MB", Format.formatNumber, row[14]);
    this.addRowIfValue(this.addPropertyRow, table, "Disk Remaining MB", Format.formatNumber, row[15]);
