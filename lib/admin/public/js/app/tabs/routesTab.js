...
RoutesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[9] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[9]), route.guid, AdminUI.showEvents);
    }

    if (row[10] != null)
    {
        this.addFilterRow(table, "Route Mappings", Format.formatNumber(row[10]), route.guid, AdminUI.showRouteMappings);
    }
