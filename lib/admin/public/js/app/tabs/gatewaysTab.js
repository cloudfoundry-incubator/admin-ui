...
GatewaysTab.prototype.updateTableRow = function(row, gateway)
{
    ...
    if (gateway.connected)
    {
        ...
        var percentAvailableCapacity = (availableCapacity > 0) ? Math.round((availableCapacity / (provisionedServices + availableCapacity)) * 100) : 0;
    ...
    else
    {
        ...
        Utilities.addEmptyElementsToArray(row, 8);
...
GatewaysTab.prototype.getGatewayProvisionedServices = function(gateway)
{
    var numServices = 0;
...
GatewaysTab.prototype.clickHandler = function()
{
    ...
    if ((selected.length > 0) && (selected[0][1] == Constants.STATUS__RUNNING))
    {
        ...
        this.addPropertyRow(table, "Provisioned Services", Format.formatNumber(selected[0][7]));
        this.addPropertyRow(table, "Available Capacity",   Format.formatNumber(selected[0][8]) + " (" + selected[0][9] + "%)");
