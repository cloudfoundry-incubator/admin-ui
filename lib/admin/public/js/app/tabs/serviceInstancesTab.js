...
ServiceInstancesTab.prototype.refresh = function(reload)
{
    ...
    $.when(applicationsDeferred, organizationsDeferred, serviceBrokersDeferred, serviceBindingsDeferred, serviceInstancesDeferred, servicePlansDeferred, servicesDeferred, spacesDeferred).done($.proxy(function(applicationsResult, organizationsResult, serviceBrokersResult, serviceBindingsResult, serviceInstancesResult, servicePlansResult, servicesResult, spacesResult)
    {
        this.updateData([applicationsResult, organizationsResult, serviceBrokersResult, serviceBindingsResult, serviceInstancesResult, servicePlansResult, servicesResult, spacesResult], reload);
...
ServiceInstancesTab.prototype.getTableData = function(results)
{
    ...
    var serviceBrokers   = results[2].response.items;
    var serviceBindings  = results[3].response.items;
    var serviceInstances = results[4].response.items;
    var servicePlans     = results[5].response.items;
    var services         = results[6].response.items;
    var spaces           = results[7].response.items;
    ...
    for (var serviceInstanceIndex in serviceInstances)
    {
        ...
        if (serviceInstance.updated_at != null)
        {
            row.push(serviceInstance.updated_at);
        }
        else
        {
            Utilities.addEmptyElementsToArray(row, 1);
...
ServiceInstancesTab.prototype.clickHandler = function()
{
    ...
    if (selected.length > 0)
    {
        ...
        if (serviceInstance.updated_at != null)
        {
            this.addPropertyRow(table, "Service Instance Updated", Format.formatDateString(serviceInstance.updated_at));
        }
        ...
        if (serviceBroker != null)
        {
            ...
            if (serviceBroker.updated_at != null)
            {
                this.addPropertyRow(table, "Service Broker Updated", Format.formatDateString(serviceBroker.updated_at));
            }
        }
        ...
        if (service != null)
        {
            ...
            if (service.updated_at != null)
            {
                this.addPropertyRow(table, "Service Updated", Format.formatDateString(service.updated_at));
            }
        ...
        if (servicePlan != null)
        {
            ...
            {
                AdminUI.showServicePlan(servicePlan.guid);
            ...
            if (servicePlan.updated_at != null)
            {
                this.addPropertyRow(table, "Service Plan Updated", Format.formatDateString(servicePlan.updated_at));
            }
...
ServiceInstancesTab.prototype.showServiceInstances = function(filter)
{
    ...
    $.when(applicationsDeferred, organizationsDeferred, serviceBrokersDeferred, serviceBindingsDeferred, serviceInstancesDeferred, servicePlansDeferred, servicesDeferred, spacesDeferred).done($.proxy(function(applicationsResult, organizationsResult, serviceBrokersResult, serviceBindingsResult, serviceInstancesResult, servicePlansResult, servicesResult, spacesResult)
    {
        var tableData = this.getTableData([applicationsResult, organizationsResult, serviceBrokersResult, serviceBindingsResult, serviceInstancesResult, servicePlansResult, servicesResult, spacesResult]);
