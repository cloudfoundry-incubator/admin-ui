...
ServicePlansTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (servicePlan != null)
    {
        ...
        $(serviceInstancesLink).html(Format.formatNumber(row[8]));
...
ServicePlansTab.prototype.changeVisibility = function(targetedVisibility) 
{
    ...
    AdminUI.showModalDialog("Managing Service Plans", $("<label>Performing operation, please wait...</label>"));

    try
    {
        for (var index = 0; index < servicePlans.length; index++) 
        {
            var servicePlan = servicePlans[index];
            ...
            {
                error_servicePlans.push(servicePlan);
            });
        }
    }
    finally
...
ServicePlansTab.prototype.getSelectedServicePlans = function() 
{
    ...
    {
        servicePlans.push(checkedRows[index].value);
    }
