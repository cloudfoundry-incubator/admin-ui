...
RouteMappingsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Protocol", Format.formatString, routeMapping.protocol);

    if ((route != null) && (row[8] != null))
    {
        if ((route.port != null) && (route.port !== 0))
        {
            this.addPropertyRow(table, "URI", Format.formatString(row[8]));
        }
        else
        {
            this.addURIRow(table, "URI", row[8]);
        }
    }
