...
ClientsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[9] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[9]), client.client_id, AdminUI.showEvents);
    }
    
    if (row[10] != null)
    {
        this.addFilterRow(table, "Approvals", Format.formatNumber(row[10]), client.client_id, AdminUI.showApprovals);
    }
