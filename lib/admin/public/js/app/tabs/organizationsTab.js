...
OrganizationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    {
        var organizationsLabelsTableData = [];
        ...
        for (var labelIndex = 0; labelIndex < labels.length; labelIndex++)
        {
            ...
            var organizationLabelRow = [];

            organizationLabelRow.push(label.resource_guid + "/labels/" + encodeURIComponent(label.key_prefix) + "/" + encodeURIComponent(label.key_name));
            organizationLabelRow.push(label.key_prefix);
            organizationLabelRow.push(label.key_name);
            organizationLabelRow.push(label.guid);
            organizationLabelRow.push(wrapper.created_at_rfc3339);
            organizationLabelRow.push(wrapper.updated_at_rfc3339);
            organizationLabelRow.push(label.value);

            organizationsLabelsTableData.push(organizationLabelRow);
        }

        // Have to show the table prior to populating for its sizing to work correctly.
        $("#OrganizationsLabelsTableContainer").show();

        this.organizationsLabelsTable.api().clear().rows.add(organizationsLabelsTableData).draw();
    }
};
