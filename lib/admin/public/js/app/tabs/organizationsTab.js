...
OrganizationsTab.prototype.getActions = function()
{
    return [
               ...
               },
               {
                   ...
                   click: $.proxy(function()
                   {
                       this.setOrganizationsStatuses("activate");
                   },
                   this)
               },
               {
                   ...
                   click: $.proxy(function()
                   {
                       this.setOrganizationsStatuses("suspend");
                   },
                   this)
               },
               {
...
OrganizationsTab.prototype.renameOrganization = function()
{
    var organization = this.getCheckedSingle();
    ...
    var dialogContentDiv = $("<div></div>");
    dialogContentDiv.append($("<label>Name: </label>"));
    dialogContentDiv.append($("<input type='text' id='organizationName' value='" + organization.name + "'>"));

    AdminUI.showModalDialogAction("Rename Organization",
                                  dialogContentDiv,
                                  "Rename",
                                  $.proxy(function()
                                  {
...
OrganizationsTab.prototype.setOrganizationsStatuses = function(type)
{
    var organizations = this.getChecked();
...
OrganizationsTab.prototype.updateOrganizations = function(organizations, controlMessage)
{
    AdminUI.showModalDialogProgress("Managing Organizations");

    var processed = 0;
    
    var errorOrganizations = [];

    for (var organizationIndex = 0; organizationIndex < organizations.length; organizationIndex++)
    {
        var organization = organizations[organizationIndex];

        var deferred = $.ajax({
                                  type:             "PUT",
                                  url:              Constants.URL__ORGANIZATIONS + "/" + organization.key,
                                  contentType:      "application/json; charset=utf-8",
                                  dataType:         "json",
                                  data:             controlMessage,
        ...
        deferred.always(function(xhr, status, error)
        {
            ...
            if (processed == organizations.length)
            {
                if (errorOrganizations.length > 0)
                {
                    AdminUI.showModalDialogErrorTable(errorOrganizations);
                }
                else
                {
                    AdminUI.showModalDialogSuccess();
