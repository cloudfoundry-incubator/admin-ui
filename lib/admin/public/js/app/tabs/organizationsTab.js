...
OrganizationsTab.prototype.getColumns = function()
{
    return [
               ...
               {
                   "sTitle":  "Private Service Brokers",
                   "sWidth":  "70px",
                   "sClass":  "cellRightAlign",
                   "mRender": Format.formatNumber
               },
...
OrganizationsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[15] != null)
    {
        this.addFilterRow(table, "Total Routes", Format.formatNumber(row[15]), target, AdminUI.showRoutes);
    }
    ...
    if (row[18] != null)
    {
        this.addFilterRow(table, "Instances Used", Format.formatNumber(row[18]), target, AdminUI.showApplicationInstances);
    }

    if (row[19] != null)
    {
        this.addFilterRow(table, "Services Used", Format.formatNumber(row[19]), target, AdminUI.showServiceInstances);
    }
    ...
    if (row[25] != null)
    {
        this.addFilterRow(table, "Total Apps", Format.formatNumber(row[25]), target, AdminUI.showApplications);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Started Apps", Format.formatNumber, row[26]);
    this.addRowIfValue(this.addPropertyRow, table, "Stopped Apps", Format.formatNumber, row[27]);
    this.addRowIfValue(this.addPropertyRow, table, "Pending Apps", Format.formatNumber, row[28]);
    this.addRowIfValue(this.addPropertyRow, table, "Staged Apps",  Format.formatNumber, row[29]);
    this.addRowIfValue(this.addPropertyRow, table, "Failed Apps",  Format.formatNumber, row[30]);
};
