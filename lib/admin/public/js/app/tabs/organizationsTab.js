...
OrganizationsTab.prototype.refresh = function(reload)
{
    ...
    $.when(applicationsDeferred, spacesDeferred, spacesDevelopersDeferred, organizationsDeferred, deasDeferred, routesDeferred).done($.proxy(function(applicationsResult, spacesResult, spacesDevelopersResult, organizationsResult, deasResult, routesResult)
    {
        this.updateData([applicationsResult, spacesResult, spacesDevelopersResult, organizationsResult, deasResult, routesResult], reload);
    },
    this));
}
...
OrganizationsTab.prototype.getTableData = function(results)
{
    ...
    for (var organizationIndex in organizations)
    {
        ...
        var routesString = (organizationRouteCounters['totalRoutes'] - organizationRouteCounters['unusedRoutes']) || 0;
        routesString += "/";
        routesString += organizationRouteCounters['totalRoutes'] || 0;
        row.push(routesString);
        ...
        if (organizationAppCounters)
        {
            ...
            row.push(organizationRouteCounters['totalRoutes']  || 0);
            row.push(organizationRouteCounters['unusedRoutes'] || 0);
        }
        else
        {
            Utilities.addZeroElementsToArray(row, 15);
        }
...
OrganizationsTab.prototype.showDetails = function(table, organization, row)
{
    ...
    $(instancesLink).html(Format.formatNumber(row[6]));
    ...
    $(servicesLink).html(Format.formatNumber(row[7]));
    ...
    $(appsLink).html(Format.formatNumber(row[13]));
    ...
    this.addPropertyRow(table, "Started Apps",    Format.formatNumber(row[14]));
    this.addPropertyRow(table, "Stopped Apps",    Format.formatNumber(row[15]));
    this.addPropertyRow(table, "Pending Apps",    Format.formatNumber(row[16]));
    this.addPropertyRow(table, "Staged Apps",     Format.formatNumber(row[17]));
    this.addPropertyRow(table, "Failed Apps",     Format.formatNumber(row[18]));

    var routeString = row[19];
    if (row[20] > 0)
    {
        routeString += ", " + row[20] + " of them have no applications bound.";
    }
    var routesLink = document.createElement("a");
    $(routesLink).attr("href", "");
    $(routesLink).addClass("tableLink");
    $(routesLink).html(Format.formatString(routeString));
...
OrganizationsTab.prototype.showOrganization = function(organizationName)
{
    ...
    $.when(applicationsDeferred, spacesDeferred, spacesDevelopersDeferred, organizationsDeferred, deasDeferred, routesDeferred).done($.proxy(function(applicationsResult, spacesResult, spacesDevelopersResult, organizationsResult, deasResult, routesResult)
    {
        var tableData = this.getTableData([applicationsResult, spacesResult, spacesDevelopersResult, organizationsResult, deasResult, routesResult]);
