...
ServicesTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[10] != null)
    {
        this.addFilterRow(table, "Service Plans", Format.formatNumber(row[10]), service.guid, AdminUI.showServicePlans);
    }
    
    if (row[11] != null)
    {
        this.addFilterRow(table, "Service Plan Visibilities", Format.formatNumber(row[11]), service.guid, AdminUI.showServicePlanVisibilities);
    }
    
    if (row[12] != null)
    {
        this.addFilterRow(table, "Service Instances", Format.formatNumber(row[12]), service.guid, AdminUI.showServiceInstances);
    }
    
    if (row[13] != null)
    {
        this.addFilterRow(table, "Service Bindings", Format.formatNumber(row[13]), service.guid, AdminUI.showServiceBindings);
    }
