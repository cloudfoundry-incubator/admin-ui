...
UsersTab.prototype.showDetails = function(table, objects, row)
{
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Password Updated", Format.formatDateString, user_uaa.passwd_lastmodified);
    ...
    this.addRowIfValue(this.addPropertyRow, table, "Active", Format.formatBoolean, user_uaa.active);
    ...
    if (row[11] != null)
    {
        this.addFilterRow(table, "Events", Format.formatNumber(row[11]), user_uaa.id, AdminUI.showEvents);
    }
    
    if (row[12] != null)
    {
        this.addFilterRow(table, "Organization Total Roles", Format.formatNumber(row[12]), user_uaa.id, AdminUI.showOrganizationRoles);
    }
    
    this.addRowIfValue(this.addPropertyRow, table, "Organization Auditor Roles", Format.formatNumber, row[13]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization Billing Manager Roles", Format.formatNumber, row[14]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization Manager Roles", Format.formatNumber, row[15]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization User Roles", Format.formatNumber, row[16]);
    
    if (row[17] != null)
    {
        this.addFilterRow(table, "Space Total Roles", Format.formatNumber(row[17]), user_uaa.id, AdminUI.showSpaceRoles);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Space Auditor Roles", Format.formatNumber, row[18]);
    this.addRowIfValue(this.addPropertyRow, table, "Space Developer Roles", Format.formatNumber, row[19]);
    this.addRowIfValue(this.addPropertyRow, table, "Space Manager Roles", Format.formatNumber, row[20]);
};
