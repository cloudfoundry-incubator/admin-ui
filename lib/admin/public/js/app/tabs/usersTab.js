...
UsersTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (groups != null)
    {
        for (var groupIndex = 0; groupIndex < groups.length; groupIndex++)
        {
            ...
            this.addFilterRow(table, "Group", Format.formatString(group), group, AdminUI.showGroups);
        }
    }
    ...
    if (row[14] != null)
    {
        this.addFilterRow(table, "Organization Total Roles", Format.formatNumber(row[14]), user.id, AdminUI.showOrganizationRoles);
    }
    
    this.addRowIfValue(this.addPropertyRow, table, "Organization Auditor Roles", Format.formatNumber, row[15]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization Billing Manager Roles", Format.formatNumber, row[16]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization Manager Roles", Format.formatNumber, row[17]);
    this.addRowIfValue(this.addPropertyRow, table, "Organization User Roles", Format.formatNumber, row[18]);
    
    if (row[19] != null)
    {
        this.addFilterRow(table, "Space Total Roles", Format.formatNumber(row[19]), user.id, AdminUI.showSpaceRoles);
    }

    this.addRowIfValue(this.addPropertyRow, table, "Space Auditor Roles", Format.formatNumber, row[20]);
    this.addRowIfValue(this.addPropertyRow, table, "Space Developer Roles", Format.formatNumber, row[21]);
    this.addRowIfValue(this.addPropertyRow, table, "Space Manager Roles", Format.formatNumber, row[22]);
};
