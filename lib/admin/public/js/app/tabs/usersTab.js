...
UsersTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if (row[14] != null)
    {
        this.addFilterRow(table, "Groups", Format.formatNumber(row[14]), user.id, AdminUI.showGroupMembers);
    }

    if (row[15] != null)
    {
        this.addFilterRow(table, "Approvals", Format.formatNumber(row[15]), user.id, AdminUI.showApprovals);
    }

    if (row[16] != null)
    {
        this.addFilterRow(table, "Organization Total Roles", Format.formatNumber(row[16]), user.id, AdminUI.showOrganizationRoles);
    }
    ...
    if (row[21] != null)
    {
        this.addFilterRow(table, "Space Total Roles", Format.formatNumber(row[21]), user.id, AdminUI.showSpaceRoles);
    }
