...
IsolationSegmentsTab.prototype.initialize = function()
{
    ...
    this.isolationSegmentsAnnotationsTable = Table.createTable("IsolationSegmentsAnnotations", this.getIsolationSegmentsAnnotationsColumns(), [[1, "asc"]], null, this.getIsolationSegmentsAnnotationsActions(), Constants.FILENAME__ISOLATION_SEGMENT_ANNOTATIONS, null, null);
};
    ...
    return [
               {
                   ...
                   render:    $.proxy(function(value, type, item)
                                      {
                                          return this.formatCheckbox("IsolationSegmentsAnnotations", item[1], value);
                                      },
                                      this)
               },
               {
...
IsolationSegmentsTab.prototype.showDetails = function(table, objects, row)
{
    ...
    if ((labels != null) && (labels.length > 0))
    {
        ...
        for (var labelIndex = 0; labelIndex < labels.length; labelIndex++)
        {
            ...
            var path = label.resource_guid + "/labels/" + encodeURIComponent(label.key_name);
    ...
    if ((annotations != null) && (annotations.length > 0))
    {
        ...
        for (var annotationIndex = 0; annotationIndex < annotations.length; annotationIndex++)
        {
            ...
            annotationRow.push(annotation.resource_guid + "/annotations/" + encodeURIComponent(annotation.key));
