...
var AdminStats =
{
    startup: function()
    {
        var historyDeferred = $.ajax({
                                  url: Constants.URL__STATS,
                                  dataType: "json",
                                  type: "GET"
                              });

        var currentDeferred = $.ajax({
                                  url: Constants.URL__CURRENT_STATS,
                                  dataType: "json",
                                  type: "GET"
                              });

        $.when(historyDeferred, currentDeferred).done(function(historyResult, currentDeferred)
        {
            historyResult[0].items.unshift(currentDeferred[0]);
            AdminStats.data = historyResult[0];
            AdminStats.initialize();
        });

        $.when(historyDeferred, currentDeferred).fail(function(xhr, status, error)
        {
