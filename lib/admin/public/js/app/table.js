...
var Table =
{
    ...
    createTable: function(tableID, columns, initialSort, clickHandler, tableActions, serverSideURL)
    {
        var tableConfig = this.getSelectableTableConfiguration(tableID, initialSort, clickHandler, tableActions, serverSideURL);

        tableConfig["aoColumns"] = columns;
        // http://www.datatables.net/plug-ins/i18n/
        // tableConfig["language"] = { "url": "https://raw.githubusercontent.com/DataTables/Plugins/master/i18n/Chinese-traditional.lang" }

        var table = $("#" + tableID + "Table").dataTable(tableConfig);
        
        // Use plugin to cause half-second delay when entering in search field
    ...
    getSelectableTableConfiguration: function(tableID, initialSort, clickHandler, tableActions, serverSideURL)
    {
        var config = {
        ...
        if (serverSideURL)
        {
            ...
            config.fnServerData  = function(sSource, aoData, fnCallback, oSettings) 
            {
                ...
                {            
                    ...
                    {
                        var errorText = i18n.t('errors.services-unavailable');
                ...
                deferred.fail(function(xhr, status, error)
                {
                    ...
                    {
                        AdminUI.showErrorPage(i18n.t('errors.services-unavailable'));
        ...
        {
            for (var step = 0; step < tableActions.length; step ++)
            {
                var tableAction = tableActions[step];
