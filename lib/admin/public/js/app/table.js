var Table =
{
    ...
    getSelectableTableConfiguration: function(tableID, columns, initialSort, clickHandler, tableActions, filename, serverSideURL, afterServerDataCallback)
    {
        ...
        if (serverSideURL)
        {
            ...
            config.ajax = function(data, callback, settings)
            {
                ...
                // Drop unused query parameters
                $.each(data.columns, function(_, column) {
        ...
        config.buttons.push({ extend: "copy", exportOptions: exportOptions });
        config.buttons.push({ extend: "print", exportOptions: exportOptions });
        ...
        config.buttons.push({
                                ...
                                "buttons":
                                [
                                    { extend: "csv",   filename: filename, exportOptions: exportOptions },
                                    { extend: "excel", filename: filename, exportOptions: exportOptions },
                                    { extend: "pdf",   filename: filename, exportOptions: exportOptions, orientation: "landscape" }
                                ]
                            });
     ...
     * This consists of the flash-based buttons.
     */
    wrapRawButtons: function(api, tableID)
    {
        ...
        for (var buttonIndex = 0; buttonIndex < buttons.length; buttonIndex++)
        {
            ...
            // TODO - assign ID's to button nodes so we can test the buttons
            node.id = buttonIDPrefix + buttonIndex;
            ...
            if ((text == "Copy") || (text == "CSV") || (text == "Excel") || (text == "PDF"))
            {
                ...
                // Need closure so the value of action is not the last value from the buttons loop
                (function(oldAction)
                {
