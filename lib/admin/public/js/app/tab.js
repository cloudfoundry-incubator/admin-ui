...
{
    var serverSideURL = null;
    if (this.serverSide)
    {
        serverSideURL = this.url;
    }
    
    this.table = Table.createTable(this.id, this.getColumns(), this.getInitialSort(), $.proxy(this.clickHandler, this), this.getActions(), serverSideURL);
};
...
Tab.prototype.refreshServerSide = function(reload)
{
    var selected   = [];
    ...
    // TODO - Both show and restoreTablePageNumber make a round trip to the server for data
...
{
    countersMap.reserved_memory += application.memory     * application.instances;
    countersMap.reserved_disk   += application.disk_quota * application.instances;
