...
Tab.prototype.refresh = function()
{
    ...
    if (selected.length > 0)
    {
        try
        {
            ...
            try
            {
                // TODO - as a workaround I have to get the node to pass to select due to DataTables/TableTools mismatch
                var row = selected[0];
                var node = this.table.fnGetNodes(row._DT_RowIndex);
                tableTools.fnSelect(node);
            }
            finally
            {
                this.refreshing = false;
...
{
    return "<input type='checkbox' name='" + escape(name) + "' value='" + value + "' onclick='Tab.prototype.checkboxClickHandler(event)' tabID='" + this.id + "'></input>";
};
...
Tab.prototype.checkboxClickHandler = function(event)
{
    ...
    var tabID         = target.getAttribute("tabID");
...
{
    var checkedRows = $("input:checked", this.table.api().rows().nodes());
...
Tab.prototype.getCheckedSingle = function()
{
    var checkedRows = $("input:checked", this.table.api().rows().nodes());
...
Tab.prototype.deleteChecked = function(prompt, okButtonText, progress, baseURL, suffix)
{
...
Tab.prototype.renameSingleChecked = function(actionTitle, progressTitle, baseURL)
{
    var object = this.getCheckedSingle();
...
Tab.prototype.updateChecked = function(progressTitle, baseURL, body)
{
    var objects = this.getChecked();
...
Tab.prototype.update = function(progressTitle, baseURL, objects, body)
{
