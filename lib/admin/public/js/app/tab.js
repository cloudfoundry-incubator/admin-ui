...
Tab.prototype.itemClicked = function(index, ignoreState)
{
    ...
    if ((selected.length > 0) && (ignoreState || (selected[0][1] == Constants.STATUS__RUNNING)))
    {
...
Tab.prototype.addInstanceMetrics = function(countersMap, application, instanceMap)
{
    ...
    if (instances != null)
    {
        countersMap.instances += instances.length
        ...
        for (var instanceIndex in instances)
        {
            ...
            if (services != null)
            {
                for (var serviceIndex in services)
                {
                    ...
                    // A service instance can be used across multiple applications and we want to ulimately count each instance only once
