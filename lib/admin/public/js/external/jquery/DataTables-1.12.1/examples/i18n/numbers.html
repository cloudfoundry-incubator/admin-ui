...
<head>
	...
	<title>DataTables example - Locale based number display</title>
...
<body class="dt-example">
	<div class="container">
		<section>
			<h1>DataTables example <span>Locale based number display</span></h1>
			<div class="info">
				<p>DataTables has a built in number rendering type that can help to display numbers in a format your end users are familiar and comfortable with. For example, in
				many parts of the world <code>.</code> is used as a thousands separator, while in other parts <code>,</code> is used. The number renderer can automatically
				determine the correct display for your end users and display them as such.</p>
				<p>The <code>DataTable.render.number</code> helper can take no arguments at all for fully automatic display, or up to five parameters for finer grained control. In
				this example the thousands and decimal separator (first two parameters) are defined as <code>null</code> to let DataTables determine what to use, zero decimal
				places and a prefix of <code>$</code>.</p>
